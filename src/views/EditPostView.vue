<template>
  <div>
  <h1>Edit posts</h1>
  <v-btn class="bg-yellow-lighten-3 pa-3 my-1" @click="$emit('close')">Close</v-btn>
   ...
   <v-card v-for="person in posts" :key="person" class="my-5 pa-5 bg-white rounded " elevation="1">
    ...
    <v-row >
      <v-col class="d-flex justify-space-between mt-3" cols="12">
        <v-btn class="bg-blue-lighten-3" @click="clickToUpdateData(person.id)">Edit</v-btn>
        <v-btn class="bg-red-lighten-3" @click="clickToDeleteData(person.id)">Delete</v-btn>
      </v-col>
